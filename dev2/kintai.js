...
(function ($) {
	'use strict';
	kintone.events.on(['space.portal.show', 'space.mobile.portal.show'], function (event) {
 	// スペースIDが10の場合のみ実行
	if (event.spaceId && event.spaceId === '10') {
		console.log("勤怠ボタンを表示する")
	...
	function initHtmlTimecard(event) {
		...
		if (mobileCheck(event.type)) {
			createElmTimecard().appendTo(kintone.mobile.space.portal.getContentSpaceElement());
		} else {
			createElmTimecard().appendTo(kintone.space.portal.getContentSpaceElement());
		}
